...
work for xpilot-web.html. This server provides the same kind of
broadcast relay, but over WebSocket + JSON, for browser clients.
...
The server also tracks each connected client's last known state so a
freshly-joined client can be sent a snapshot of everyone already in
the game (a "roster" message) instead of waiting for their next state
broadcast.
...
port (default 8766) for quick diagnostics, mirroring net_server.py.
"""
import argparse
import asyncio
import json
